...
// '''#################################################################
// DEMO 为【左臂：设置笛卡尔阻力控相关参数 + 切换扭矩模式 + 读回校验】的示例
// 目的：
//     1) 下发左臂笛卡尔刚度阻尼参数（K/D）与控制类型（
//     2) 下发末端笛卡尔旋转方向参数
//     3) 设置关节速度/加速度比例限制
//     4) 切换左臂目标状态到扭矩相关模式，并设置阻抗类型
//     5) 通过 OnGetBuf 读取 DCSS，打印当前状态命令状态错误码，以及下发参数是否生效
//

// 参数说明（按当前示例写法）：
//     - K[7] / D[7]：左臂笛卡尔阻抗刚度/阻尼7维数组
//     - type：力控阻抗类型选择（示例中 type=1；具体含义以 SDK 文档为准
//     - Dir[7]：笛卡尔方向/旋转参数：前三个通常为基于基于X/Y/Z 的旋转量，后四个保留参数填0
//
// '''#################################################################
...
int main()
{
    ...
    // 设置笛卡尔阻抗参数
    double K[7] = {6000, 6000, 6000, 100, 100, 100, 20}; // 设置刚度和阻尼参数
    double D[7] = {0.1, 0.1, 0.1, 0.3, 0.3, 0.3, 1};

    int type = 3;                       // 设置左臂末端阻抗类型fcType=4
    double Dir[7] = {0, 0, 0, 0, 0, 0}; // 笛卡尔方向：CartCtrlPara前三个参数置为末端基于基座X Y Z顺序的旋转，后四个为保留参数，填0

    double target_joint[7] = {9, 50, 10, -115, 0, 0, 0};

    std::cout << "The robotic arm will move from the current position to the following position: 9, 50, 10, -115, 0, 0, 0" << std::endl;
    std::cout << "Please ensure environmental safety!" << std::endl;
    std::cout << "Should it be continued? Y or N " << std::endl;
    std::string input;
    std::cin >> input;
    if (input != "Y" && input != "y")
    {
    ...
    OnClearSet();
    OnSetForceCtrPara_A(3, fxDir, fcCtrlPara, fcAdjLmt);
