...
import logging
'''#################################################################
该DEMO 为设置控制器时间和控制器主板重启案例.
    注意：设置时间和重启是两个独立功能，时间设置不需要重启。
...
'''设置系统时间'''
ret=robot.set_system_time(2026,7,18,17,0,0)
if ret:
     logger.info('success:robot set time')
else:
    logger.error('failed:robot set time failed')
    exit(0)
time.sleep(1)
