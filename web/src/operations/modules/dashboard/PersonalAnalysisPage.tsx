...
type Machine = Record<string, any> & { machineNumber: string; _runtimeStatus?: RuntimeStatus };
...
function statusOf(machine: Machine): RuntimeStatus {
  ...
  if (machine.collectorStarted === false || machine.edgeContainerRoleStatus?.collector?.running === false
    || machine.containerRoleStatus?.collector?.running === false) return 'online_idle';
...
export default function PersonalAnalysisPage() {
  ...
  useEffect(() => {
    if (!detailMachine) { setDetail(null); setOperationsCenter(null); setDetailTab('overview'); return; }
    const controller = new AbortController();
    api.getMachineInfo(detailMachine).then(setDetail).catch(() => {});
    api.getMachineOperationsCenter(detailMachine).then(setOperationsCenter).catch(() => setOperationsCenter(null));
    api.streamMachineLive(detailMachine, setDetail, controller.signal).catch(() => {});
  ...
  const machines = useMemo(() => {
    ...
    return Object.values(map).map(machine => ({ ...machine, _runtimeStatus: statusOf(machine) }))
      .sort((a, b) => naturalCompare(a.machineNumber, b.machineNumber));
  }, [baseMachines, liveMap]);

  const counts = useMemo(() => {
    const value: Record<string, number> = { all: machines.length, error: 0, recording: 0, online_idle: 0, offline: 0 };
    machines.forEach(machine => { value[machine._runtimeStatus!] += 1; });
    return value;
  }, [machines]);
  ...
  const visible = useMemo(() => {
    ...
    return machines.filter(machine => {
      if (statusFilter !== 'all' && machine._runtimeStatus !== statusFilter) return false;
  ...
  const renderCard = (machine: Machine) => {
    ...
    const productionStatus = status === 'recording'
      ? 'in_production'
      : (machine.productionStatus || 'ready');
    const task = taskOf(machine);
    const operator = operatorOf(machine);
      ...
      >
        <div className="ow-card-top">
          ...
          <span className="ow-status-stack">
            <span className="ow-production-status">{PRODUCTION_LABEL[productionStatus] || '在生产'}</span>
        ...
        <div className="ow-detail-mask" onMouseDown={e => { if (e.target === e.currentTarget) setDetailMachine(null); }}>
          <aside className="ow-detail ow-detail-wide">
            <div className="ow-detail-head">
              <div className="ow-detail-title"><small>机器状态中心 / 实时详情</small><h2>{detailMachine}</h2><span>{detail?.source === 'agent' ? `Agent 心跳 · ${detail.dataAgeSec ?? '?'} 秒前` : 'Importer 实时读取'}</span></div>
              <div className="ow-detail-actions"><Button icon={<ReloadOutlined />} onClick={() => { api.getMachineInfo(detailMachine).then(setDetail).catch(() => {}); api.getMachineOperationsCenter(detailMachine).then(setOperationsCenter).catch(() => {}); }}>刷新</Button><button aria-label="关闭" onClick={() => setDetailMachine(null)}>×</button></div>
            </div>
              ...
              <div className="ow-detail-body">
                ...
                <section className="ow-center-summary">
                  ...
                  <div className="ow-center-grid">
                    ...
                    <div><small>Collector</small><b>{detail.collectorAlive == null ? '-' : detail.collectorAlive ? '存活' : '停止'}</b></div>
